...
const parsedGrid = computed(() => {
    ...
    props.schedules.forEach(sched => {
        ...
        if (grid[start] && grid[start][roomId] !== undefined) {
            ...
            // Mark covered slots for this room
                                ...
                                <template v-else>
                                    <template v-for="room in sortedRooms" :key="room.id">
                                        ...
                                        >
                                            ...
                                            >
                                                <div class="font-extrabold text-xs leading-snug text-slate-900 break-words">
                                                    {{ parsedGrid.grid[slotNum][room.id].mata_kuliah.nama }} {{ parsedGrid.grid[slotNum][room.id].kelas.nama_kelas }}
                                                </div>
                                                <div class="mt-2 flex flex-col gap-1 text-[10px] text-slate-500">
                                                    <span class="gap-1">
                                                        Dosen: <strong class="text-slate-700 font-semibold">{{ parsedGrid.grid[slotNum][room.id].dosen.nama }}</strong>
                                                    </span>
                                                </div>
                                            </div>
                                        </td>
